...
if($controlClass=='BasicForm' && isset($ctrl_params) && is_array($ctrl_params)) {
    ...
    if(strlen($fTagId) && strlen($fResponseTarget)) {
        ...
        if($isModal) {
            ...
            $ctrl_params['actions'][] = array(
                'params'=>array('tag_id'=>'df_'.$tName.'_back','value'=>Translate::GetButton('back'),'icon'=>'fa fa-chevron-left','class'=>NApp::$theme->GetBtnDefaultClass(),'onclick'=>NApp::Ajax()->LegacyPrepare("AjaxRequest('{$cModule}','{$cMethod}','id_template'|{$idTemplate}~'id'|{$idInstance},'{$cTarget}')->{$cTarget}")),
            );
        }//if($isModal)
    }//if(strlen($fTagId) && strlen($fResponseTarget))
}//if($controlClass=='BasicForm' && isset($ctrl_params) && is_array($ctrl_params))