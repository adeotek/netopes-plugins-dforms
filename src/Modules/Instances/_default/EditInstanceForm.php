	...
	if(isset($ctrl_params) && is_array($ctrl_params)) {
		...
		if(strlen($fTagId) && strlen($fResponseTarget)) {
			...
			if($is_modal) {
				...
				$ctrl_params['actions'][] = array(
                    'params'=>array('tag_id'=>'df_'.$tName.'_back','value'=>Translate::GetButton('back'),'icon'=>'fa fa-chevron-left','class'=>'btn btn-default','onclick'=>NApp::Ajax()->LegacyPrepare("AjaxRequest('{$cModule}','{$cMethod}','id_template'|{$idTemplate}~'id'|{$idInstance},'{$cTarget}')->{$cTarget}")),
				);
			}//if($is_modal)
		}//if(strlen($fTagId) && strlen($fResponseTarget))
