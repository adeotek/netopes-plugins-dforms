    ...
    if(strlen($fTagId) && strlen($fResponseTarget)) {
        ...
        if($params->safeGet('back_action',TRUE,'bool')) {
            if($this->formsAsModal) {
                ...
                $ctrl_params['actions'][]=[
                    'params'=>['value'=>Translate::GetButton('back'),'icon'=>'fa fa-chevron-left','class'=>NApp::$theme->GetBtnDefaultClass(),'onclick'=>NApp::Ajax()->Prepare("{ 'module': '{$cModule}', 'method': '{$cMethod}', 'params': { 'id_template': {$this->templateId}, 'target': '{$cTarget}' } }",$cTarget)],
                ];
            }//if($this->formsAsModal)
        }//if($params->safeGet('back_action',TRUE,'bool'))
    }//if(strlen($fTagId) && strlen($fResponseTarget))
