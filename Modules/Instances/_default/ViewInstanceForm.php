<?php
	...
	$vcontent = get_array_param($instance,'print_template','','is_string');
	if(strlen($vcontent)) {
		...
		if(is_array($fields) && count($fields)) {
			foreach($fields as $field) {
				...
				$fval = get_array_param($field,'ivalues','','is_string');
		...
		if(is_array($relations) && count($relations)) {
			foreach($relations as $rel) {
				$fname = get_array_param($rel,'name','','is_string');
				if(!strlen($fname)) { continue; }
				$fval = get_array_param($rel,'svalues',get_array_param($rel,'ivalues','','is_string'),'is_string');
				$vcontent = str_replace('['.$fname.']',$fval,$vcontent);
			}//END foreach
		}//if(is_array($relations) && count($relations))
	} else {
