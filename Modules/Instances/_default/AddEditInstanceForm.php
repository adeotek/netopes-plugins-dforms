	...
	if(isset($ctrl_params) && is_array($ctrl_params)) {
		...
		if(strlen($f_tagid) && strlen($f_rtarget)) {
			$ctrl_params['actions'] = array(
				array(
					'params'=>array('tag_id'=>'df_'.$tname.'_save','value'=>Translate::GetButton('save'),'icon'=>'fa fa-save','class'=>NApp::$theme->GetBtnPrymaryClass(),'onclick'=>NApp::arequest()->Prepare("AjaxRequest('{$this->name}','SaveRecord','id_template'|{$id_template}~'id'|{$id_instance}~'data'|df_{$tname}_form:form~'is_modal'|'{$is_modal}'~'cmodule'|'{$cmodule}'~'cmethod'|'{$cmethod}'~'ctarget'|'{$ctarget}','{$f_tagid}')->{$f_rtarget}")),
				),
			);
			if($is_modal) {
				$ctrl_params['actions'][] = array(
				...
				$ctrl_params['actions'][] = array(
					'params'=>array('tag_id'=>'df_'.$tname.'_back','value'=>Translate::GetButton('back'),'icon'=>'fa fa-chevron-left','class'=>NApp::$theme->GetBtnDefaultClass(),'onclick'=>NApp::arequest()->Prepare("AjaxRequest('{$cmodule}','{$cmethod}','id_template'|{$id_template}~'id'|{$id_instance},'{$ctarget}')->{$ctarget}")),
				);
			}//if($is_modal)
		}//if(strlen($f_tagid) && strlen($f_rtarget))
