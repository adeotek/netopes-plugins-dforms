...
use NETopes\Core\Controls\TextBox;
	...
	switch($cclass) {
		...
		case 'Message':
			...
			$hfields[] = $ctrl->Show();
			$ctrl = new HiddenInput(['tag_id'=>'dft_fp_name','tag_name'=>'name','value'=>$item->getProperty('name',\PAF\AppSession::GetNewUID($idTemplate.$cclass),'is_string')]);
			$hfields[] = $ctrl->Show();
			if($cclass=='FormTitle') {
	...
	$ctrl_params['actions'] = [
	    [
	        'params'=>['value'=>Translate::GetButton('save'),'icon'=>'fa fa-save','onclick'=>NApp::arequest()->Prepare("AjaxRequest('{$this->name}','AddEditContentElementRecord',
